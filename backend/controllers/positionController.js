...
const setPosition = asyncHandler(async(req, res) => {
    ...
    pluralName = pluralName[0].toLocaleUpperCase()+pluralName.slice(1).toLocaleLowerCase()
    singularName = singularName[0].toLocaleUpperCase()+singularName.slice(1).toLocaleLowerCase()
...
const updatePosition = asyncHandler(async (req, res) => {
    ...
    if(!position) {
        ...
        Object.keys(req.body).forEach(val => {
            if(val === 'pluralName') {
                req.body.pluralName = req.body.pluralName[0].toLocaleUpperCase()+req.body.pluralName.slice(1).toLocaleLowerCase()
            }
            if(val === 'singularName') {
                req.body.singularName = req.body.singularName[0].toLocaleUpperCase()+req.body.singularName.slice(1).toLocaleLowerCase()
            }
            if(val === 'shortName') {
        ...
        res.status(200).json({msg: `${updatedPosition.singularName} field updated`})
    }
})
