...
const ManagerLive = require('../models/managerLive')
    ...
    for(let i=0; i<allPicks.length; i++) {
        ...
        if(mLive === null) {
            await ManagerLive.create({
                ...
                livePicks: [{matchday:id, matchdayId:mid,
                     activeChip:null, picks:allPicks[i].picks}]
            })
        } else {
            const mLivePicks = mLive.livePicks
            let idIndex = mLive.livePicks.findIndex(x => x.matchday === id)
            let midIndex = mLive.livePicks.findIndex(X => X.matchdayId.toString() === mid.toString())
            if(idIndex > -1 || midIndex > -1) {
                res.status(400)
                throw new Error('Matchday scores already exist')
            }
            const newLivePicks = [...mLivePicks, {matchday:id, matchdayId:mid, activeChip:null, picks:allPicks[i].picks}]
            await ManagerLive.findOneAndUpdate({user: allPicks[i].user}, 
                {livePicks:newLivePicks}, {new: true})
        }
...
const setInitialPoints = asyncHandler(async(req, res) => {
    ...
    const players = await Player.find({$or: [{playerTeam: teamAway}, {playerTeam: teamHome}]})
    if(!fixture) {
        res.status(400)
    ...
    for(let i=0; i<players.length; i++) {
        ...
        for(let i=0; i<allLives.length; i++) {
            ...
            const a = mLivePicks
            .filter(x => x.matchday === +req.params.mid)
            a.length > 0 && a[0].picks.map(x => x._id.toString() === _id.toString() ? x.points = mdPoints : x.points)

            await ManagerLive.findOneAndUpdate({user: allLives[i].user}, 
                {livePicks:mLivePicks}, {new: true})
        }
    }
...
const addPointsToPicks = asyncHandler(async(req, res) => {
    ...
    for(let i=0; i<allLives.length; i++) {
        ...
        const a = mLivePicks
        .filter(x => x.matchdayId.toString() === req.params.mid)
        a.length > 0 && a[0].picks.map(x => x._id.toString() === req.params.pid.toString() ? x.points = mdPoints : x.points)
...
const deletePoints = asyncHandler(async(req, res) => {
    ...
    for(let i=0; i<players.length; i++) {
        ...
        for(let i=0; i<allLives.length; i++) {
            ...
            const a = mLivePicks
            .filter(x => x.matchdayId.toString() === mid.toString())
            a[0].picks.map(x => x._id.toString() === _id.toString() ? x.points = null : x.points)
...
const getRound = asyncHandler(async(req, res) => {
    ...
    const rlivePicks = livePicks.find(live => live.matchday === +req.params.mid)
...
module.exports = {getLivePicks, getRound, setLivePicks, setInitialPoints, addPointsToPicks, deletePoints}